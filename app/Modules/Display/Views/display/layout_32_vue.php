        ...
        <div class="row">
            ...
            <div class="col-6">
                <div v-if="dataCuaca != null">
                    <h4 class="placeholder-glow" v-if="loadingCuaca == true">
                    ...
                    <div v-for="item in dataCuaca.weather" :key="item.id" v-else>
                        <h4 class="fw-normal mb-0">
                            {{ item.main }}, {{ item.description }} <img class="mb-0" :src="'http://openweathermap.org/img/wn/' + item.icon + '.png'" height="30">
                        </h4>
                    </div>
                </div>
                <div v-else>
...
<?php foreach ($custom as $row) { ?>
    <?php if ($row['konten'] == 'jadwalsholat') : ?>
        <div class="position-fixed bottom-0 end-0 p-2" style="z-index: 11; bottom: 10% !important;">
            <div id="liveToast" class="toast align-items-center text-dark border-0" :class="toast2" role="alert" aria-live="assertive" aria-atomic="true" style="width: 250px;">
