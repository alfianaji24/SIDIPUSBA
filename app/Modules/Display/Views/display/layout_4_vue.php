            ...
            <div class="card bg-dark text-white transparan border-0 mb-3">
                ...
                <div class="card-body">
                    <div v-if="dataCuaca != null">
                        ...
                        <p class="mb-0" v-else>Feels like {{ Math.floor(dataCuaca_main.feels_like) }}&deg;<span>C</span>. Humidity {{ dataCuaca_main.humidity }}%</p>
                        <small class="text-muted">API openweathermap.org</small>
                    </div>
                    <div v-else>
                    ...
                    <div v-else>
                        ...
                        <carousel :autoplay="true" :per-page="1" :loop="true" :autoplay-timeout="<?= $vc_autoplaytimeout; ?>">
                            <slide v-for="(item, i ) in dataAgenda" :key="i" :data-index="i" class="mb-4">
                                <h4 class="fw-bold">{{ item.nama_agenda }}, {{ item.tgl_agenda }}</h4>
                                <h4>Tempat: {{ item.tempat_agenda }}</h4>
                                <h4>Waktu: {{ item.waktu }} - Selesai</h4>
                            </slide>
                        </carousel>
                    </div>
                </div>
