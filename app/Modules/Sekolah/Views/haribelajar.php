                    ...
                    <tbody>
                        <?php foreach ($dataJam as $jam) { ?>
                            <tr>
                                ...
                                $builder = $db->table('haribelajars');
                                $builder = $builder->where(['id_jampelajaran' => $jam['id']]);
                                $query = $builder->get();
                                ...
                                foreach ($query->getResult() as $row) {
                                    $id_hari = $row->id;
                                    $senin = kegiatan($row->senin)['id'];
                                    $selasa = kegiatan($row->selasa)['id'];
                                    $rabu = kegiatan($row->rabu)['id'];
                                    $kamis = kegiatan($row->kamis)['id'];
                                    $jumat = kegiatan($row->jumat)['id'];
                                    $sabtu = kegiatan($row->sabtu)['id'];
                                }
                                ?>
                                <td class="<?= ($senin == "Istirahat" ? "yellow" : ""); ?>"><?= (isset($senin) ? $senin : ""); ?> </td>
                                <td class="<?= ($selasa == "Istirahat" ? "yellow" : ""); ?>"><?= (isset($selasa) ? $selasa : ""); ?></td>
                                <td class="<?= ($rabu == "Istirahat" ? "yellow" : ""); ?>"><?= (isset($rabu) ? $rabu : ""); ?></td>
                                <td class="<?= ($kamis == "Istirahat" ? "yellow" : ""); ?>"><?= (isset($kamis) ? $kamis : ""); ?></td>
                                <td class="<?= ($jumat == "Istirahat" ? "yellow" : ""); ?>"><?= (isset($jumat) ? $jumat : ""); ?></td>
                                <td class="<?= ($sabtu == "Istirahat" ? "yellow" : ""); ?>"><?= (isset($sabtu) ? $sabtu : ""); ?></td>
                                <td>
...
<script>
    ...
    methodsVue = {
        ...
        getJam: function() {
            this.loading = true;
            axios.get(`<?= base_url(); ?>api/jampelajaran/${this.idJam}`)
                .then(res => {
                    // handle success
                    this.loading = false;
                    var data = res.data;
        ...
        getHari: function() {
            this.loading = true;
            axios.get(`<?= base_url(); ?>api/haribelajar/${this.idHari}`)
                .then(res => {
                    // handle success
                    this.loading = false;
                    var data = res.data;
        ...
        saveHari: function() {
            this.loading = true;
            axios.post(`<?= base_url(); ?>api/haribelajar/save`, {
                    id_jampelajaran: this.idJam,
                    senin: this.senin,
                    selasa: this.selasa,
                    rabu: this.rabu,
                    kamis: this.kamis,
        ...
        updateHari: function() {
            this.loading = true;
            axios.put(`<?= base_url(); ?>api/haribelajar/update/${this.idHari}`, {
                    id_jampelajaran: this.idJam,
                    senin: this.senin,
                    selasa: this.selasa,
                    rabu: this.rabu,
